...
public:
    ...
    // 构造对象
    void constructe(T* addr) { new (addr) T; }
    // 析构对象
    void destructe(T* addr, int size) {
...
public:
    ...
    ~vector() {
        // delete[] first_;
        // 只对有效元素进行析构
        alloc_.destructe(first_, last_ - first_);
    ...
    vector<T>& operator=(const vector<T>& other) {
        ...
        // 对有效元素进行析构
        alloc_.destructe(first_, last_ - end_);
        // 释放内存
